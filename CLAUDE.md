...
- **The waitlist is live via Kit (ConvertKit).** Both forms POST the email to `https://app.kit.com/forms/9559731/subscriptions` (field `email_address`) from the submit handler in `teaser.js`; the endpoint is the `KIT_FORM_ENDPOINT` constant near the top of that file. **Kit returns HTTP 200 even on failure**, so the handler branches on the JSON `status` field (`'failed'` = error), not `res.ok`. `localStorage['ww_waitlist']` is now only a "don't ask twice" flag that re-shows the success seal for returning visitors. **Double opt-in is enabled in Kit**, so subscribers must click a confirmation email before they're on the list — that's why the success-seal copy says "check your inbox." (To make signup instant, turn off double opt-in in the Kit form settings.)
- **The share actions are still client-side only.** Share buttons open intent URLs or copy hardcoded links (`https://wherewolf.app`, `https://discord.gg/wherewolf`) defined in `teaser.js`.
